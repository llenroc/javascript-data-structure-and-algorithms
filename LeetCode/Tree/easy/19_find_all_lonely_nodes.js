...
var getLonelyNodes = function(root) {
    const pushNodesToStack = (root, stack) => {
      while(root !== null) {
        stack.push(root);
        root = root.left;
      }
    ...
    const results = [];
    const stack = [];
    pushNodesToStack(root, stack);
    while(stack.length) {
      const top = stack.pop();
      if(top.right) {
        pushNodesToStack(top.right, stack);
      }
      if(!top.left && top.right) {
        results.push(top.right.val);
      }
      if(top.left && !top.right) {
        results.push(top.left.val);
      }
    }
...
console.log(getLonelyNodes(root)); // [4]


root = [7,1,4,6,null,5,3,null,null,null,null,null,2];
root = createCompleteBinaryTreeFromArray(root);
console.log(getLonelyNodes(root)); // [6,2]

root = [11,99,88,77,null,null,66,55,null,null,44,33,null,null,22];
root = createCompleteBinaryTreeFromArray(root);
console.log(getLonelyNodes(root)); // [77,55,33,66,44,22]
...
root = createCompleteBinaryTreeFromArray(root);
console.log(getLonelyNodes(root)); // [78,28]